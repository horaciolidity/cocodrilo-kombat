...
export function MissionsView({
  ...
  const [validatingMissions, setValidatingMissions] = React.useState({}); // { missionId: { startTime: number, endTime: number } }

  // 📝 Cargar estado de validación local al iniciar
  React.useEffect(() => {
    try {
      const savedValidation = localStorage.getItem('mission_validations');
      if (savedValidation) {
        setValidatingMissions(JSON.parse(savedValidation));
      }
    } catch (e) {
      console.error("Error loading mission validations", e);
    }
  }, []);

  // ⏱️ Verificar temporizadores cada segundo
  React.useEffect(() => {
    const interval = setInterval(() => {
      const now = Date.now();
      let updated = false;
      const newValidations = { ...validatingMissions };
  ...
  }, [validatingMissions, missions, completeMission, playSound, toast]);
  ...
  const startYoutubeValidation = (mission) => {
    ...
    const newValidations = {
      ...validatingMissions,
      [mission.id]: { startTime: now, endTime }
    };
        ...
        <div className="space-y-6">
          {(() => {
            ...
            const sortedMissions = [...activeMissions].sort((a, b) => {
              ...
              const getPriority = (m) => {
                if (m.validation_type === 'daily_code') return 100;
                if (m.validation_type === 'video_watch' || m.validation_type === 'youtube_actions') return 90;
                if (m.category === 'social') return 80;
                return 0;
              };
                        ...
                        <div className="flex flex-col gap-2 w-full">
                          ...
                          {validatingMissions[mission.id] && (
                            <div className="text-center p-2 bg-yellow-900/20 rounded border border-yellow-500/30">
                              ...
                              >
                                ⏳ Validando suscripción, like y comentarios...
                              </motion.p>
                              <p className="text-[10px] text-yellow-200/70 mt-1">
                                Tiempo restante: {Math.max(0, Math.ceil((validatingMissions[mission.id].endTime - Date.now()) / 60000))} min
                              </p>
                            </div>
                          )}
