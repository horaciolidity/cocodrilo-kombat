...
export function FarmingMilestonesView({ 
  ...
  const renderMilestoneCard = (milestone, index) => {
    ...
    if (milestone.tokenReward >= 1000) {
      milestoneTier = 'legendary';
      tierColor = 'text-yellow-400';
      tierBgColor = 'bg-yellow-900/30';
    } else if (milestone.tokenReward >= 500) {
      milestoneTier = 'epic';
    ...
    return (
      <motion.div
        key={milestone.id}
        className={`milestone-card rounded-xl p-4 md:p-6 shadow-lg transition-all duration-300 relative overflow-hidden ${
          state.claimed 
            ...
            {state.claimed ? (
              ...
              <div className="text-center p-2 bg-gray-800/30 rounded-lg">
                ...
                <div className="text-xs text-gray-500">
                  Por alcanzar {milestone.coinsRequired.toLocaleString()} monedas
                </div>
              </div>
            )}
          </div>
